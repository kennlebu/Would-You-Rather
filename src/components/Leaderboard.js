...
class Leaderboard extends Component {
    ...
    render() {
        ...
        const users_scored = Object.keys(users)
            .map((uid) => (
                Object.assign({}, users[uid], {'score': Object.keys(users[uid].answers).length + users[uid].questions.length})
            ))
                        ...
                        <li key={user.id}>
                            <div className='leaderboard'>
                                ...
                                <div className='leaderboard-picture'>
                                    <img src={user.avatarURL} alt={user.name} width='100px' height='100px'/><br/>
                                    <span>{user.name}</span>
                                </div>
